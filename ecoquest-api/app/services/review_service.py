...
class ReviewService:
    ...
    async def create_review(self, data: ReviewCreate, reviewer: User) -> Review:
        ...
        try:
            ...
            new_status = SubmissionStatus.APPROVED if data.decision.lower() in ("approved", "verified") else SubmissionStatus.REJECTED
            ...
            logger.info("Created review %s for submission %s by reviewer %s", created_review.review_id, data.submission_id, reviewer.user_id)
            return created_review

        except Exception as exc:
            logger.error("Transaction error during review creation, rolling back: %s", exc)

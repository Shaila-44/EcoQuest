...
class UserRepository(BaseRepository[User]):
    ...
    async def get_by_id(self, id: uuid.UUID) -> Optional[User]:
        ...
        stmt = select(User).options(selectinload(User.role)).where(User.user_id == id)
        result = await self.session.execute(stmt)
        return result.scalar_one_or_none()

    async def get_by_email_hash(self, email_hash: str) -> Optional[User]:
        """Fetch a user by email hash with role eager-loaded."""
        stmt = select(User).options(selectinload(User.role)).where(User.email_hash == email_hash)
        result = await self.session.execute(stmt)
        return result.scalar_one_or_none()
